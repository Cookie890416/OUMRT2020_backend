...
@app.route('/query_event/<string:event_id>/<string:user_id>')
def query_event(event_id,user_id):
    if event_id and user_id:
        current_event = db.current_collection.find({"event_id": event_id})
        users=db.user_collection.find({"user_id": user_id})
        x=[]
        if current_event and users:
            for i in current_event:
                i.pop("_id")
                x.append(actual_start_point:+i["actual_start_point"])
                
                x.append(actual_end_point:+i["actual_end_point"])
                
                x.append(actual_time:+i["actual_time"])
                
                x.append(is_self_helmet:+i["is_self_helmet"])
                
                x.append(extra_needed:+i["extra_needed"])
            for i in users:
                i.pop("_id")
               
                x.append(user_id:+i["user_id"])
                
                x.append(phone_num:+i["phone_num"])
